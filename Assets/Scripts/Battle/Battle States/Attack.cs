...
public class Attack : BattleState
{
    ...
    const int DAMAGE_BY_CARD_DEATH = 1;
    ...
    public override void ExecuteAction()
    {
        if (!clickedEndTurnBtn && !clickedRepositionAgainBtn)
        {
            ...
            if (ReceivedValidInput())
            {
                ...
                {
                    playerAttacker.TakeDamage(DAMAGE_BY_CARD_DEATH);
                }    
                ...
                {
                    targetPlayer.TakeDamage(amountOfEnemyCardsThatJustDied * DAMAGE_BY_CARD_DEATH);
                }
